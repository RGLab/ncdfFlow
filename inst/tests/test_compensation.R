comp.fs <- read.flowSet(path = system.file("extdata","compdata","data",package="flowCore"))
comp.fs <- ncdfFlowSet(comp.fs)
test_that("compensate", {
      ...
      expect_equal(fsApply(comp.fs1, colMeans, use.exprs = TRUE), expectRes[["trans.comp"]])
